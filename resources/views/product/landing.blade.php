...
<body class="font-sans antialiased bg-white">
            ...
            <div class="relative" style="height: 500px;">
                ...
                <div class="absolute inset-0 flex items-center justify-center">
                    <div class="text-center px-4 max-w-4xl">
                        <h1 class="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-white mb-4 leading-tight">{{ $landingPage->hero_title }}</h1>
                        @if($landingPage->hero_subtitle)
                            <p class="text-lg sm:text-xl text-gray-200 mb-8 max-w-2xl mx-auto">{{ $landingPage->hero_subtitle }}</p>
                        @endif
        ...
        @else
            <div class="bg-gradient-to-br from-indigo-600 via-indigo-700 to-purple-800 pt-20 pb-20 sm:pt-28 sm:pb-28">
                <div class="max-w-4xl mx-auto text-center px-4">
                    <h1 class="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-white mb-4 leading-tight">{{ $landingPage->hero_title }}</h1>
                    @if($landingPage->hero_subtitle)
                        <p class="text-lg sm:text-xl text-indigo-100 mb-8 max-w-2xl mx-auto">{{ $landingPage->hero_subtitle }}</p>
                    @endif
    ...
    <section class="py-16 sm:py-20">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 class="text-3xl font-bold text-gray-900 text-center mb-8">Tentang Produk</h2>
    ...
    <section class="py-16 sm:py-20 bg-gray-50">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
                ...
                <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
                    <div class="text-sm text-gray-500 mb-1">Komisi Affiliator</div>
                    <div class="text-3xl font-bold text-green-600">{{ $product->commission_percent }}%</div>
                </div>
                <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
                    <div class="text-sm text-gray-500 mb-1">Bonus Upline</div>
                    <div class="text-3xl font-bold text-blue-600">{{ $product->upline_percent }}%</div>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="py-16 sm:py-20 bg-gray-50">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 class="text-3xl font-bold text-gray-900 text-center mb-4">Apa Kata Mereka</h2>
                ...
                @foreach($product->landingPageTestimonials as $testimonial)
                    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
                        <div class="flex items-center gap-0.5 mb-3">
                            @for($i = 1; $i <= 5; $i++)
                                <svg class="w-5 h-5 {{ $i <= $testimonial->rating ? 'text-yellow-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                            @endfor
                        </div>
                        <p class="text-gray-600 text-sm mb-4 leading-relaxed">{{ $testimonial->content }}</p>
                        <div class="flex items-center gap-3 border-t border-gray-100 pt-4">
                            ...
                            @else
                                <div class="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center flex-shrink-0">
                                    <span class="text-indigo-600 font-semibold">{{ strtoupper(substr($testimonial->name, 0, 1)) }}</span>
                                </div>
                            @endif
                            <div>
